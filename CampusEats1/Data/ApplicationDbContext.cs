...
public class ApplicationDbContext : DbContext
{
    ...
    public DbSet<AktivnostLog> Aktivnosti => Set<AktivnostLog>();
    public DbSet<HistorijaIzmjene> HistorijaIzmjena => Set<HistorijaIzmjene>();
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<Rezervacija>()
            .HasOne(rezervacija => rezervacija.Korisnik)
            .WithMany(korisnik => korisnik.Rezervacije)
            .HasForeignKey(rezervacija => rezervacija.KorisnikId)
            .OnDelete(DeleteBehavior.Restrict);

        modelBuilder.Entity<Rezervacija>()
            .HasOne(rezervacija => rezervacija.Kurir)
            .WithMany(korisnik => korisnik.Dostave)
            .HasForeignKey(rezervacija => rezervacija.KurirId)
            .OnDelete(DeleteBehavior.Restrict);

        modelBuilder.Entity<Obavijest>()
            .HasOne(obavijest => obavijest.Korisnik)
            .WithMany(korisnik => korisnik.Obavijesti)
            .HasForeignKey(obavijest => obavijest.KorisnikId)
            .OnDelete(DeleteBehavior.SetNull);

        modelBuilder.Entity<Obavijest>()
            .HasOne(obavijest => obavijest.Rezervacija)
            .WithMany()
            .HasForeignKey(obavijest => obavijest.RezervacijaId)
            .OnDelete(DeleteBehavior.SetNull);
